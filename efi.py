...
# Para Guardar
    ...
    def guardaPart(self, evt):
        ...
        ############################################################### PARA VER SI YA UN TURNO ESTA RESERVADO O NO
        con = sqlite3.connect("clientes1")
        cur = con.cursor()
        par = (horario, fecha)
        cur.execute("SELECT * FROM datos WHERE horario=? AND fecha=?", par) # Consulto si ya hay registro para esa fecha y hora
        consulta = cur.fetchall()
        if len(consulta) > 0:
            wx.MessageBox("La fecha y el horario ya esta reservado") # Para que veas desde la linea de comandos
        else:
            alta = f"INSERT INTO datos VALUES (NULL, '{apellido}', '{nombre}', '{numero}', '{fecha}', '{servicio}','{horario}','{precio}')" # Faltaba comilla simple al final de precio
            wx.MessageBox("Fecha asignada")
            cur.execute(alta)
            con.commit()
            self.dvlc.AppendItem([apellido, nombre, numero, fecha, servicio, horario , precio]) # verificar que no esté duplicado
...
#-BUSCAR TURNO POR FECHA --------------------------------------------------------------------------------- FRAME 2 ----------------------------------------------
    ...
    def clientes_por_fecha(self,h):
        ...
        self.lb1 = ListBox(panel3, size=(300, 400)) # declaro la listbox donde irán los agendados para hoy
        grilla3.Add(self.lb1, flag=EXPAND|ALL, border=10) #
...
#-----------------------------Metodo de busqueda de turnos -----------------------------------------------------#
    ...
    def clientesdehoy(self,datos):
        ...
        for tup in valores: 
            tupLis = list(tup) 
            tupLis[2] = str(tupLis[2]) 
            turno = ' '.join(tupLis)

        print(turno)
        self.lb1.SetItems(turno) # agrego los agendados de fecha de hoy al ListBox
        # LA VARIABLE "valores" ES LA QUE TIENE LOS VALORES DE LO CLEINTES

        if len(turno) == 0: # no va el try porque no va dar error aunque devuelva cero registros
            print("No hay turnos para esta fecha")
            self.lb1.SetItems(['No hay turnos de esta fecha']) #
